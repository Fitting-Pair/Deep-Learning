...
async def process_image(userId: int = Form(...), file: UploadFile = File(...)):
    ...
    keypoints_file = keypoints_dir / f"{file.filename.replace('.jpg','')}_keypoints.json"
    # 2. SMPLify 실행
    ...
    # 결과 obj 파일 경로
    obj_file = smplify_results_dir / "meshes" / file.filename.replace('.jpg', '') / "000.obj"
    obj_dir = smplify_results_dir / "meshes" / file.filename.replace('.jpg', '')
    ...
    print(response_data)
    #obj.png file
    obj_png= "/Users/vecherish/Desktop/gradsmu/smplifyx/content/data/smplify_results/meshes_png/000.png"
    #obj_png = smplify_results_dir / "meshes_png" / "000.png"
    multipart_data = {
        "json":(None,response_data,"application/json"),
        "file":("obj.png",obj_png,"image/png")
    }
    #전송
    multipart_response=requests.post(
        'http://localhost:8080/get/json', #수정
